...
const quarterDetails: Record<string, { label: string; desc: string; landmarks: string }> = {
  URO: {
    label: "Uro Quarter",
...
export default async function HomePage() {
  const { schedules, totalReports, resolvedReports } = await getStatsAndSchedules();
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      <header className="sticky top-0 z-40 w-full border-b bg-background/80 backdrop-blur-md">
        <div className="container mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium">
            <a href="#schedules" className="text-muted-foreground hover:text-foreground transition-colors">
              Pickup Schedules
            </a>
            <a href="#quarters" className="text-muted-foreground hover:text-foreground transition-colors">
              Quarters
            </a>
      ...
      <section className="relative overflow-hidden pt-12 pb-16 md:pt-20 md:pb-24 bg-linear-to-b from-emerald-500/10 via-background to-background">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6">
          <div className="flex flex-col items-center text-center max-w-3xl mx-auto">
            <Badge variant="outline" className="mb-4 px-3 py-1 text-xs border-emerald-600/30 text-emerald-700 bg-emerald-50 dark:bg-emerald-950/40 dark:text-emerald-300">
              Official Ikere-Ekiti Refuse & Sanitation Portal
            </Badge>
            ...
            <p className="mt-5 text-base sm:text-lg text-muted-foreground max-w-2xl text-balance">
              Empowering residents across Uro, Oke-Osun, Odo-Oja, Ogbontioro, and Olowo-Ijesa to report illegal dumping, track weekly collection schedules, and maintain environmental hygiene.
            </p>

            <div className="mt-8 flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
              <Button asChild size="lg" className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm font-semibold">
            ...
            <div className="mt-14 w-full grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-2xl border bg-card/60 backdrop-blur-xs shadow-xs text-left">
              <div className="p-3">
                <div className="text-2xl font-bold text-foreground">5 Quarters</div>
                <div className="text-xs text-muted-foreground mt-0.5">Municipal Ikere coverage</div>
              </div>
              <div className="p-3">
                <div className="text-2xl font-bold text-emerald-600">{totalReports > 0 ? totalReports : 28}+</div>
                <div className="text-xs text-muted-foreground mt-0.5">Community reports processed</div>
              </div>
              <div className="p-3">
                <div className="text-2xl font-bold text-foreground">{resolvedReports > 0 ? resolvedReports : 24}+</div>
                <div className="text-xs text-muted-foreground mt-0.5">Evacuated dumpsites</div>
              </div>
              <div className="p-3">
                <div className="text-2xl font-bold text-emerald-600">100%</div>
                <div className="text-xs text-muted-foreground mt-0.5">Scheduled weekly dispatch</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="schedules" className="py-16 border-t bg-muted/20">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6">
          <div className="max-w-2xl mx-auto text-center mb-12">
            <Badge variant="secondary" className="mb-2">Weekly Timetable</Badge>
            <h2 className="text-3xl font-bold tracking-tight">Sanitation & Collection Schedules</h2>
            <p className="mt-3 text-muted-foreground text-sm sm:text-base">
              Trucks and environmental crews operate systematically across all five major quarters. Ensure domestic refuse bins are placed by the roadside before 07:00 AM on your scheduled day.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {schedules.map((schedule) => {
              ...
              return (
                <Card key={schedule.id} className="relative border shadow-xs hover:border-emerald-500/50 transition-all duration-200">
                  <CardHeader className="pb-3">
                    ...
                    <CardTitle className="text-lg mt-2">{quarterInfo.label}</CardTitle>
                  ...
                  <CardContent className="space-y-3 text-xs">
                    <div className="p-2.5 rounded-lg bg-muted/60 flex items-start gap-2">
                      ...
                      <div>
                        <span className="font-semibold block text-foreground">Assigned Crew:</span>
                        <span className="text-muted-foreground">{schedule.crewAssigned || "Ikere Sanitation Patrol"}</span>
                      </div>
                    </div>
          ...
          <div className="mt-8 p-4 rounded-xl border bg-card text-center flex flex-col sm:flex-row items-center justify-between gap-4 max-w-3xl mx-auto shadow-xs">
            <div className="text-left text-xs sm:text-sm">
              <span className="font-semibold block text-foreground">Missed collection or overflowing bin?</span>
              <span className="text-muted-foreground">Submit a quick report and our response team will be alerted immediately.</span>
            </div>
      ...
      <section id="how-it-works" className="py-16 border-t">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6">
          <div className="max-w-2xl mx-auto text-center mb-12">
            <Badge variant="secondary" className="mb-2">Simple 3-Step Process</Badge>
            <h2 className="text-3xl font-bold tracking-tight">How Community Reporting Works</h2>
            <p className="mt-3 text-muted-foreground text-sm sm:text-base">
              Direct municipal accountability powered by OpenStreetMap GPS coordinates and photographic verification.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="p-6 rounded-2xl border bg-card shadow-xs flex flex-col items-start text-left">
              <div className="flex size-11 items-center justify-center rounded-xl bg-emerald-50 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300 font-bold mb-4">
                1
              </div>
              <h3 className="text-lg font-bold">Snap & Pin Location</h3>
              <p className="text-sm text-muted-foreground mt-2 leading-relaxed">
                Take a photograph of the waste accumulation and pin the exact coordinates on our interactive map of Ikere-Ekiti.
              </p>
            </div>
            ...
            <div className="p-6 rounded-2xl border bg-card shadow-xs flex flex-col items-start text-left">
              ...
              <p className="text-sm text-muted-foreground mt-2 leading-relaxed">
                The waste administration reviews the report and dispatches the quarter&apos;s sanitation truck and collection crew.
              </p>
            </div>
            ...
            <div className="p-6 rounded-2xl border bg-card shadow-xs flex flex-col items-start text-left">
              ...
              <p className="text-sm text-muted-foreground mt-2 leading-relaxed">
                Refuse is cleared and disposed at designated municipal dumpsites, with real-time status updates visible to the reporter.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Footer */}
      <footer className="mt-auto border-t bg-card py-10">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-2 font-bold text-sm">
            ...
            <span>Ikere-Ekiti Waste Management System</span>
          </div>

          <p className="text-xs text-muted-foreground text-center md:text-right">
            Final Year Engineering Project &copy; {new Date().getFullYear()} Ikere-Ekiti Sanitation Authority. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
