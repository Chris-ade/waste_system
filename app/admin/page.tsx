  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <Card className="border shadow-xs bg-linear-to-t from-primary/5 to-card">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardDescription className="text-xs">Total Incidents Reported</CardDescription>
        ...
        <Card className="border shadow-xs bg-linear-to-t from-amber-500/5 to-card">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardDescription className="text-xs">Pending Dispatch</CardDescription>
        ...
        <Card className="border shadow-xs bg-linear-to-t from-blue-500/5 to-card">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardDescription className="text-xs">Active Crew Dispatches</CardDescription>
        ...
        <Card className="border shadow-xs bg-linear-to-t from-emerald-500/5 to-card">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <CardTitle className="text-2xl font-bold text-emerald-600">{resolutionRate}%</CardTitle>
      ...
      <Card className="border shadow-xs">
        <CardHeader className="pb-3 border-b">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-base">Quarter Breakdown across Ikere-Ekiti</CardTitle>
              <CardDescription className="text-xs">
                Incident density and reports logged per municipal administrative quarter
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent className="p-4">
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            {[
              ...
              { code: "OKE_OSUN", name: "Oke-Osun", count: quarterData.OKE_OSUN },
              { code: "ODO_OJA", name: "Odo-Oja", count: quarterData.ODO_OJA },
              { code: "OGBONTIORO", name: "Ogbontioro", count: quarterData.OGBONTIORO },
              { code: "OLOWO_IJESA", name: "Olowo-Ijesa", count: quarterData.OLOWO_IJESA },
            ].map((q) => (
              <div key={q.code} className="p-3 rounded-xl border bg-muted/20 text-center space-y-1">
                <span className="text-[11px] font-semibold text-muted-foreground block truncate">{q.name}</span>
                <span className="text-xl font-bold text-foreground">{q.count}</span>
                <span className="text-[10px] text-muted-foreground block">incidents</span>
              </div>
            ))}
          </div>
        </CardContent>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold tracking-tight">Active GIS Incident Locations</h2>
          </div>
          <Link href="/admin/map" className="text-xs text-emerald-600 hover:underline flex items-center gap-1 font-semibold">
      ...
      <Card className="border shadow-xs">
        <CardHeader className="flex flex-row items-center justify-between border-b pb-4">
          <div>
            <CardTitle className="text-base">Recent Incident Submissions</CardTitle>
            <CardDescription className="text-xs">Latest reports logged by Ikere residents</CardDescription>
          </div>
        ...
        <CardContent className="p-0">
          <div className="divide-y">
            {recentReports.map((report) => (
              <div key={report.id} className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs hover:bg-muted/10 transition-colors">
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-3 text-muted-foreground text-[11px]">
                    ...
                    <span>•</span>
                    <span>Reporter: {report.user?.name || "Guest Resident"}</span>
                    <span>•</span>
